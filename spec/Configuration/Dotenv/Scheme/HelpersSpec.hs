...
spec = do
  describe "matchVarWithType" $ do
    ...
    context "when the config doesn't have the env" $
      it "should fail because it can't match the type" $
        let envConfs = [EnvConf EnvBool [Env "NODOTENV" True]]
  ...
  describe "mapMatchVarWithType" $ do
    ...
    context "when there are extra envs in the scheme" $
      it "maps the env with the specific type" $
        let envConfs = [ EnvConf EnvBool [Env "DOTENV" True]
                       , EnvConf EnvInteger [Env "OTHER_ENV" True]
                       , EnvConf EnvInteger [Env "EXTRA_ENV" True]
                       ]
            config = Config envConfs
            env = [("DOTENV", "true"), ("OTHER_ENV", "123")]
         in mapMatchVarWithType config env `shouldBe` [("true", EnvBool), ("123", EnvInteger)]

    context "when there are envs in the scheme that are not required" $
      it "maps only required envs with the specific type" $
        let envConfs = [ EnvConf EnvBool [Env "DOTENV" True]
                       , EnvConf EnvInteger [Env "OTHER_ENV" False]
                       , EnvConf EnvInteger [Env "EXTRA_ENV" False]
                       ]
            config = Config envConfs
            env = [("DOTENV", "true"), ("OTHER_ENV", "123"), ("EXTRA_ENV", "http")]
