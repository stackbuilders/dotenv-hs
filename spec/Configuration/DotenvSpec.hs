...
spec = do
  ...
  describe "loadFile" $ before_ setupEnv $ after_ clearEnvs $  do
    it "loads the configuration options to the environment from a file" $ do
      ...
      void $ loadFile $ Config ["spec/fixtures/.dotenv"] [] False False False
    ...
    it "overrides predefined settings when overload is true" $ do
      ...
      void $ loadFile $ Config ["spec/fixtures/.dotenv"] [] True False False

      lookupEnv "DOTENV" `shouldReturn` Just "true"

    context "when the .env.example is present" $ do
      let config = Config ["spec/fixtures/.dotenv"] ["spec/fixtures/.dotenv.example"] False False False
  ...
  describe "onMissingFile" $ after_ clearEnvs $ do
    context "when target file is present" $
      it "loading works as usual" $ do
        void $ onMissingFile (loadFile $ Config ["spec/fixtures/.dotenv"] [] True False False) (return [])
        lookupEnv "DOTENV" `shouldReturn` Just "true"

    context "when target file is missing" $
      it "executes supplied handler instead" $
        onMissingFile (True <$ loadFile (Config ["spec/fixtures/foo"] [] True False False)) (return False)
          `shouldReturn` False

  describe "onDuplicatedKeys" $ after_ clearEnvs $ do
    context "when target file has duplicated key and override is true" $
      it "throws an error" $
        loadFile (Config ["spec/fixtures/.dotenv", "spec/fixtures/.dotenv"] [] True False True)
          `shouldThrow` anyIOException
    context "when target file has duplicated key and override is true" $
      it "throws an error" $
        loadFile (Config ["spec/fixtures/.dotenv", "spec/fixtures/.dotenv"] [] False False True)
          `shouldThrow` anyIOException
